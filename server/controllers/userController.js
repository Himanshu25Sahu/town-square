...
export const deleteMyPost = async (req, res) => {
  ...
  try {
    ...
    // 5. Delete cloudinary attachments if they exist
    if (post.attachments && post.attachments.length > 0) {
      // Delete each attachment from cloudinary
      const deletePromises = post.attachments.map(attachment => {
        if (attachment.publicId) {
          return deleteFromCloudinary(attachment.publicId).catch(error => {
